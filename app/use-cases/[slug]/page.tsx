...
export default async function UseCaseDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <main className={styles.page}>
    ...
    <nav className={styles.useCaseNext} aria-label="Explore other use cases"><span>Explore another situation</span><div>{useCases.filter(other => other.slug !== item.slug).slice(0, 3).map(other => <Link key={other.slug} href={`/use-cases/${other.slug}`}><small>{other.number}</small>{other.title}<b>→</b></Link>)}</div></nav>
