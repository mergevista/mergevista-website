...
export default function InsightsPage() { return <main className={styles.page}>
  ...
  <section className={styles.featured}>
    <div className={styles.featuredCopy}><small>FEATURED INSIGHT · AI IN IT M&amp;A</small><h2>Where AI Actually Belongs in IT M&amp;A</h2><p>AI can reduce the work of assembling and reconciling transaction information—while experienced practitioners remain accountable for the decisions.</p><div><span>Sep 17, 2026</span><i/><span>7 min read</span></div><Link href="/insights/where-ai-belongs-in-it-ma">Read the insight <b>→</b></Link></div>
    <div className={styles.framework}><div className={styles.frameworkTop}><span>THE AI OPPORTUNITY</span><b>INFORMATION → JUDGMENT</b></div><div className={styles.lifecycle}>{["Collect","Extract","Normalize","Reconcile","Connect","Review","Decide"].map((stage,index)=><div key={stage}><i>{String(index+1).padStart(2,"0")}</i><span>{stage}</span></div>)}</div><p>SOURCE EVIDENCE · CONFIDENCE · HUMAN ACCOUNTABILITY</p></div>
  </section>
