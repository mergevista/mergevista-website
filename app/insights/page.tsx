...
export default function InsightsPage() { return <main className={styles.page}>
  ...
  <section className={styles.featured}>
    <div className={styles.featuredCopy}><small>FEATURED INSIGHT · POST-CLOSE EXECUTION</small><h2>Day 1 is not independence: What happens after legal close?</h2><p>Why legal close establishes continuity, not independence—and how transaction teams can use the TSA period to remove seller dependencies safely.</p><div><span>Sep 2, 2026</span><i/><span>13 min read</span></div><Link href="/insights/day-1-is-not-independence">Read the insight <b>→</b></Link></div>
    <div className={styles.framework}><div className={styles.frameworkTop}><span>THE POST-CLOSE JOURNEY</span><b>DAY 1 → EXIT</b></div><div className={styles.lifecycle}>{["Continuity","Stabilize","Build","Migrate","Validate","TSA exit","Independent"].map((stage,index)=><div key={stage}><i>{String(index+1).padStart(2,"0")}</i><span>{stage}</span></div>)}</div><p>DEPENDENCIES · OWNERSHIP · EVIDENCE · ACCEPTANCE</p></div>
  </section>
