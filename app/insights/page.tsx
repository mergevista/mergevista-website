...
export default function InsightsPage() { return <main className={styles.page}>
  ...
  <section className={styles.featured}>
    <div className={styles.featuredCopy}><small>FEATURED INSIGHT · APPLICATION STRATEGY</small><h2>Application disposition: Where separation strategy becomes execution</h2><p>Why disposition is more than an inventory field—and how each decision shapes data, infrastructure, contracts, TSAs, migration waves and operational readiness.</p><div><span>Sep 7, 2026</span><i/><span>12 min read</span></div><Link href="/insights/application-disposition-separation-strategy-execution">Read the insight <b>→</b></Link></div>
    <div className={styles.framework}><div className={styles.frameworkTop}><span>THE DISPOSITION DECISION</span><b>STRATEGY → EXECUTION</b></div><div className={styles.lifecycle}>{["Capability","Dependencies","Options","Decision","Plan","Evidence","Complete"].map((stage,index)=><div key={stage}><i>{String(index+1).padStart(2,"0")}</i><span>{stage}</span></div>)}</div><p>DATA · INFRASTRUCTURE · CONTRACTS · TSA · MIGRATION</p></div>
  </section>
