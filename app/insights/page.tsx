...
const articles = [
  ["AI IN IT M&A", "Where AI actually belongs in IT M&A", "Where AI adds practical value by organizing evidence and preparing decisions—while experienced practitioners remain accountable for the outcome.", "7 min read", "cyan", "/insights/where-ai-belongs-in-it-ma", "Sep 17, 2026"],
  ["MIGRATION READINESS", "Everything is green—so why isn’t the migration wave ready?", "Why workstream-level completion can still produce an operationally unready migration—and how evidence, dependencies and business-process testing change the go/no-go decision.", "11 min read", "navy", "/insights/migration-wave-readiness", "Sep 11, 2026"],
...
export default function InsightsPage() { return <main className={styles.page}>
  ...
  <section className={styles.featured}>
    <div className={styles.featuredCopy}><small>FEATURED INSIGHT · SELLER READINESS</small><h2>Your company is ready to sell. Is your technology ready for diligence?</h2><p>What lower-middle-market companies should understand, organize and be prepared to explain before the buyer begins technology diligence.</p><div><span>Sep 23, 2026</span><i/><span>11 min read</span></div><Link href="/insights/technology-ready-for-diligence">Read the insight <b>→</b></Link></div>
    <div className={styles.framework}><div className={styles.frameworkTop}><span>THE DILIGENCE QUESTION</span><b>TECHNOLOGY → TRANSACTION</b></div><div className={styles.lifecycle}>{["Inventory","Business","Contracts","Security","Data","Costs","Integrate"].map((stage,index)=><div key={stage}><i>{String(index+1).padStart(2,"0")}</i><span>{stage}</span></div>)}</div><p>KNOWN ENVIRONMENT · VISIBLE RISK · CREDIBLE EVIDENCE</p></div>
  </section>
