...
export default function InsightsPage() { return <main className={styles.page}>
  ...
  <section className={styles.featured}>
    <div className={styles.featuredCopy}><small>FEATURED INSIGHT · IT M&amp;A STRATEGY</small><h2>The complete IT M&amp;A lifecycle: From discovery to TSA exit</h2><p>Seven connected stages. Two sides of the transaction. One operating model for maintaining continuity, accountability and evidence throughout the deal.</p><div><span>Aug 27, 2026</span><i/><span>10 min read</span></div><Link href="/insights/complete-it-ma-lifecycle">Read the insight <b>→</b></Link></div>
    <div className={styles.framework}><div className={styles.frameworkTop}><span>THE IT M&amp;A LIFECYCLE</span><b>01—07</b></div><div className={styles.lifecycle}>{["Discovery","Assessment","Separation","Day 1","TSA ops","Migration","TSA exit"].map((stage,index)=><div key={stage}><i>{String(index+1).padStart(2,"0")}</i><span>{stage}</span></div>)}</div><p>CONNECTED DATA · DECISIONS · OWNERSHIP · EVIDENCE</p></div>
  </section>
